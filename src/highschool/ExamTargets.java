 ...
 * @author FRED
 */
public class ExamTargets extends JFrame implements ActionListener {
    ...
    private FredButton view = new FredButton("Save Target");
    ...
    private FredLabel number = new FredLabel("Target Points");
    ...
    private FredLabel grade = new FredLabel("Target Grade");
    ...
    public ExamTargets() {
        ...
        setTitle("Exam Target Entry Window");
        ...
        jnumber.setFont(new Font("serif", Font.BOLD, 18));
        ...
        add(jstream);
        term.setBounds(30, 150, 150, 30);
        add(term);
        jterm.setBounds(150, 150, 200, 30);
        add(jterm);
        academicyear.setBounds(400, 150, 150, 30);
        add(academicyear);
        jacademicyear.setBounds(500, 150, 200, 30);
        add(jacademicyear);
        //  examname.setBounds(30,180,150,30);add(examname);
        // jexame.setBounds(150,180,200,30);add(jexame);
        //examcode.setBounds(400,180,150,30);add(examcode);
        // jexamcode.setBounds(500,180,200,30);add(jexamcode);

        subject.setBounds(30, 250, 120, 30);
        add(subject);
        jsubject.setBounds(100, 250, 200, 30);
        add(jsubject);
        jsubjectcode.setBounds(400, 250, 150, 30);
        add(jsubjectcode);
        number.setBounds(40, 300, 200, 30);
        add(number);
        jnumber.setBounds(160, 300, 100, 30);
        add(jnumber);
        grade.setBounds(350, 300, 200, 30);
        add(grade);
        jgrade.setBounds(500, 300, 100, 30);
        add(jgrade);

        cancel.setBounds(100, 350, 130, 50);
        add(cancel);

        view.setBounds(400, 350, 200, 50);
        add(view);
        ...
        view.addActionListener(this);//mean.addActionListener(this);
        ...
        jnumber.addKeyListener(new KeyAdapter() {
            public void keyTyped(KeyEvent key) {
                ...
                if (Character.isAlphabetic(c) || jnumber.getText().length() > 3) {
                    key.consume();
                }
            }
        ...
        jgrade.addKeyListener(new KeyAdapter() {
            ...
            public void keyReleased(KeyEvent key) {
                ...
                try {
                    ...
                    rs = ps.executeQuery();
                    if (rs.next()) {
                        jnumber.setText(rs.getString("Points"));
                    } else {
                        jnumber.setText("");
                    }
        ...
        jnumber.addKeyListener(new KeyAdapter() {
            ...
            public void keyReleased(KeyEvent key) {
                ...
                try {
                    String sql = "Select grade from points_for_each_grade where  points='" + jnumber.getText() + "'";
                    ps = con.prepareStatement(sql);
                    rs = ps.executeQuery();
                    if (rs.next()) {
                    ...
                    if (jacademicyear.getSelectedIndex() == 0) {
                        ...
                        try {
                            if (jnumber.getText().isEmpty()) {
                                JOptionPane.showMessageDialog(this, "Enter Valid pionts");
                            } else {
                                if (jgrade.getText().isEmpty()) {
                                    JOptionPane.showMessageDialog(this, "Enter A Valid Grade");
                                } else {

                                    ps = con.prepareStatement("Select * from examTargets where classcode='" + Globals.classCode(jclass.getSelectedItem().toString()) + "' and subjectcode='" + jsubjectcode.getSelectedItem() + "' and academicyear='" + jacademicyear.getSelectedItem() + "'");
                                    rs = ps.executeQuery();
                                    if (rs.next()) {
                                        ps = con.prepareStatement("Update examTargets set grade='" + jgrade.getText() + "',points='" + jnumber.getText() + "' where classcode='" + Globals.classCode(jclass.getSelectedItem().toString()) + "' and subjectcode='" + jsubjectcode.getSelectedItem() + "' and academicYear='" + jacademicyear.getSelectedItem() + "'");
                                        ps.execute();
                                        JOptionPane.showMessageDialog(this, "Exam Target Updated Successfully");
                                    } else {
                                        ps = con.prepareStatement("Insert into examtargets values('" + jsubjectcode.getSelectedItem() + "','" + jnumber.getText() + "','" + jgrade.getText() + "','" + Globals.classCode(jclass.getSelectedItem().toString()) + "','" + jacademicyear.getSelectedItem() + "')");
                                        ps.execute();
                                        JOptionPane.showMessageDialog(this, "Target Set Successfully");
                                    }
                                }
                            }
                        } catch (Exception sq) {
