 ...
 * @author FRED
 */
public class SmsBom extends JFrame implements ActionListener {
    ...
    private JCheckBox smsAll = new JCheckBox("SMS All BOM Members");
    ...
    public SmsBom() {
        ...
        try {
            String sql2 = "Select fullname from Bog_Table order by fullname";
            ...
            if (message.getText().equalsIgnoreCase("")) {
                ...
                if (model2.isEmpty()) {
                    if (smsAll.isSelected()) {
                        try {
                            int counter = 0;
                            String sql = "Select fullname from bog_table";
...
//MessageGateway.silentOneForeignMessageQueue(phone, message.getText());
                                    counter++;
                    ...
                    try {
                        ...
                        for (int h = 0; h < model2.getSize(); ++h) {
                            String sql = "Select * from bog_table where fullname='" + model2.getElementAt(h).toString() + "'";
                            ps = con.prepareStatement(sql);
                            rs = ps.executeQuery();
                            if (rs.next()) {
                                name = rs.getString("fullname");
