...
public class EditBook extends JFrame implements ActionListener {
    ...
    public EditBook() {
        ...
        try {
            ...
            ps = con.prepareStatement("Select * from books where bookserial='" + Globals.bookSerial + "'");
            rs = ps.executeQuery();
            if (rs.next()) {

                jbookname.setText(rs.getString("BookName"));
                jserial.setText(rs.getString("Bookserial"));
                jclassl.setSelectedItem(Globals.className(rs.getString("Classcode")));
                jsubject.setSelectedItem(Globals.subjectName(rs.getString("Subjectcode")));
                jcondition.setSelectedItem(rs.getString("condition"));
                jtitle.setText(rs.getString("BookTitle"));
                jtype.setSelectedItem(rs.getString("type"));
                jauthor.setText(rs.getString("Author"));
                        ...
                        if (jsubject.getSelectedIndex() == 0) {
                            ...
                            if (jtype.getSelectedIndex() == 0) {
                                ...
                                if (jserial.getText().isEmpty()) {
                                    ...
                                    String prefixcode = jclassl.getSelectedItem().toString().charAt(0) + "" + jclassl.getSelectedItem().toString().charAt(5) + "/" + jsubject.getSelectedItem().toString().substring(0, 3).toUpperCase();
                                    String Serial = prefixcode + jserial.getText().substring(6);
                                    ...
                                    try {
                                        if (Globals.Level.equalsIgnoreCase("Admin")) {
                                            ...
                                            String sql = "update Books set bookname='" + jbookname.getText().toUpperCase() + "',booktitle='" + jtitle.getText().toUpperCase() + "',bookserial='" + Serial + "',author='" + jauthor.getText().toUpperCase() + "',type='" + jtype.getSelectedItem().toString().toUpperCase() + "',status='" + jcondition.getSelectedItem().toString().toUpperCase() + "',DateBought='" + DATE + "',subjectcode='" + Globals.subjectCode(jsubject.getSelectedItem().toString()).toUpperCase() + "',classcode='" + Globals.classCode(jclassl.getSelectedItem().toString()).toUpperCase() + "' where bookserial='" + Globals.bookSerial + "'";
                                            ps = con.prepareStatement(sql);
                                            ps.execute();
                                            JOptionPane.showMessageDialog(this, "Book Details Update Successfully");
                                            jbookname.setText("");
                                            jtitle.setText("");
                                            jserial.setText("");
                                            jdate.setDate(null);
                                            ...
                                            if (RightsAnnouncer.subjectright(Globals.subjectCode(jsubject.getSelectedItem().toString()), Globals.classCode(jclassl.getSelectedItem().toString()), Globals.empcode)) {
                                                ...
                                                JOptionPane.showMessageDialog(this, "You Do Not Have Sufficient Rights To Edit A book this Class");
                                            }
                                        }
